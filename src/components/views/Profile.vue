...
onMounted(() => {
  ...
  checkAuthStatus() // Initialize navbar auth state
  ...
  // 3. Load Saved Schools from LocalStorage
...
const saveProfile = () => {
  ...
  if (usersArrayStr) {
    ...
    // Also update the currentUser string!
    const currentUserStr = localStorage.getItem('currentUser')
    if (currentUserStr) {
      const currentUser = JSON.parse(currentUserStr)
      currentUser.fullName = editForm.value.fullName
